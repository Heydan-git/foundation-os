...
## Insights cette session (2026-04-20 D-VULGARIZE-01 Vulgarisation integrale)

**En bref (pour Kevin)** : Trois choses que j'ai comprises aujourd'hui. (1) Les regles ambigues se font interpreter litteralement (j'ai lu "En bref en tete" comme "seulement En bref") — donc il faut ecrire les regles de maniere explicitement couvrante. (2) Les sous-agents sont une zone d'ombre systematique : on leur delegue des taches mais on oublie de leur passer les regles de communication. (3) La discipline seule ne suffit pas pour une regle critique — il faut un garde-fou automatique qui detecte les oublis.

- **Les regles doivent etre ecrites de maniere "capture maximale"**. La regle precedente disait "tout brief commence par En bref (pour Kevin)". Techniquement correct mais ambigu : est-ce que ca veut dire "le document entier vulgarise" ou "juste l'En bref d'entree" ? J'ai pris la 2e interpretation (la plus economique en effort). Le piege : une regle qui laisse la porte ouverte a une interpretation economique sera toujours interpretee economiquement par un LLM qui optimise le cout cognitif. Fix structurel : nouveau principe P-42 ecrit avec redondance maximale — "TOUT le document, PAS juste En bref, chaque tuile, chaque section, chaque ligne". Corollaire : audit des autres regles de CLAUDE.md pour detecter les memes ambiguites (candidat futur).
- **Les sous-agents sont des trous noirs cognitifs si pas briefes explicitement**. 6 sous-agents Foundation OS (dev/doc/os-architect/review/po/alignment-auditor) produisent du contenu pour Kevin quand le primary delegue. Aucun d'eux n'avait la regle vulgarisation. Gap critique : le primary applique la regle mais le sous-agent la contourne par defaut. Pattern general : **toute regle comportementale du primary doit etre retransmise explicitement aux sous-agents via leur frontmatter**. Candidat future : script `audit-subagent-alignment.sh` qui verifie que chaque sous-agent a les memes regles critiques que le primary (P-42, P-01, P-02, P-15, P-20, etc.).
- **Garde-fou automatique > discipline pour regles a fort enjeu TDAH**. Kevin est TDAH, il oublie. Je suis Claude, je peux oublier (meme 2h apres avoir codifie une regle, j'ai commence a rederiver en jargon dans le commit log initial). Sans verification automatique, la regle P-42 aurait regresse au bout de 3 sessions. Fix : `scripts/vulgarization-check.sh` chainé dans health-check. Pattern general : **pour chaque regle comportementale critique, creer un check automatique qui scan le contenu produit**. Candidats futurs : check "pas de mensonge" (detecter metriques sans commande de verification), check "cause racine avant fix" (detecter commits fix sans mention de la cause), check "1 session a la fois" (detecter 2+ worktrees actifs). D-ENFORCE-01 pattern recurrent.

## Insights precedents (2026-04-20 Reality Check + cleanup multi-session)

**En bref (pour Kevin)** : J'ai confirme que ton inquietude etait juste. Multi-session = dette tech et memoire. L'outil Foundation OS est SAIN structurellement, mais la couche de memoire (CONTEXT.md, hot.md) se desynchronise des que 2+ sessions travaillent en parallele. La regle claire pour ne plus se perdre : **1 session a la fois, et verifier l'etat propre avant de fermer**.

- **Le multi-worktree etait un faux-ami**. Il promettait parallelisation (travailler 3 sujets en meme temps). En realite : regression memoire garantie car `/session-end` n'integre que la session qui ferme en dernier. Les autres branches sont orphelines jusqu'a audit manuel. Le cout cognitif de "retrouver ce qui est perdu" > le gain de parallelisation. **YAGNI pour dev solo** (comme D-CONCURRENCY-01 B-lite a deja conclu, mais la regle n'avait pas ete assez durcie).
- **CONTEXT.md est la source de verite la plus fragile du systeme**. Elle se modifie a chaque session, et elle contient des metriques (counts wiki, lignes fichiers, status modules). Si quelqu'un claim "rapport 864L" sans preuve, ca pollue pour longtemps (864L claim dans CONTEXT.md 2 fois, hot.md 1 fois, sessions-recent 1 fois = 4 endroits, realite 324L = 2.67x inflation). **Chaque metrique doit avoir une commande de verification** (P-16 constitution, mais pas enforce). Candidat enforcement : hook pre-commit qui verifie les claims numeriques `\dL`.
- **Le working tree desync est un silent killer**. `git status` peut rester "desync" plusieurs sessions sans que personne s'en rende compte car `/session-end` ne regarde pas systematiquement les untracked ou les mv non-committes. Hook `auto-archive-plans.sh` suppose que git add sera fait manuellement. Solution : `/session-end` doit faire un `git status --short` + warn si non-vide + bloquer si `?? ` untracked projet-niveau (pas state runtime).
- **Les projets Cowork etaient invisibles a main**. 6 dossiers `docs/travaux-cowork/` (Gmail centralisation, Morning Intelligence, Briefs Foundation OS, Project Instructions v3, etc.) n'etaient pas `gitignored` mais n'avaient jamais ete `git add`. Kevin les voyait sur son filesystem, moi je les voyais a Explore, mais la memoire main niait leur existence. Parfait cas du "travail perdu" percu. Maintenant committes.
- **L'audit reality-check etait necessaire mais consommateur**. ~3-4h travail + 10 commits + 490L rapport + 113 wiki pages integrees. Cost eleve. Mais il a calibre la regle D-NO-MULTI-SESSION-01 empiriquement. Prochaine etape : **vulgarisation systemique E8** pour que Kevin puisse lire les briefs sans jargon tech, se rappeler des decisions prises (TDAH-friendly), et me donner des directions claires. La vulgarisation doit etre CODIFIEE dans specs canoniques (CLAUDE.md + communication.md + planner.md + commands + templates), pas juste dans cette session.

## Insights precedents (2026-04-19/20 D-AUDIT-TOTAL-01 COMPLET 14/14 + D-MODEL-01)
