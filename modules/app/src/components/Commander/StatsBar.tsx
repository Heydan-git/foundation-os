...
export function StatsBar({ sessions, decisions, risks, nextSteps, source }: StatsBarProps) {
  ...
  return (
    <div className="flex flex-wrap items-center" style={{ gap: 8, marginBottom: 4 }}>
      <Pill value={sessions.length}   label="Sessions"  color="var(--color-accent-brand-secondary)" />
      <Pill value={decisions.length}  label="ADR"       color="var(--color-accent-info)" />
      <Pill value={openRisks}         label="Risques"   color="var(--color-accent-danger)" />
      <Pill value={nextSteps.length}  label="NextSteps" color="var(--color-accent-success)" />
      <Pill value={todoPct}           label="% done"    color="var(--color-accent-brand-primary)" />
      <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
        <span style={{
          width: 6, height: 6, borderRadius: '50%',
          background: source === 'supabase' ? 'var(--color-accent-brand-primary)' : 'var(--color-accent-danger)',
          display: 'inline-block',
          animation: source === 'supabase' ? 'pulse 3s infinite' : 'none',
        }} />
        <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 9, color: 'var(--color-text-faint)', letterSpacing: '.08em' }}>
